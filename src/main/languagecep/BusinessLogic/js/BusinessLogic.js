...
const path = require('path');
 ...
 class BusinessLogic {
    ...
    pasteLanguagesMaterials(workFolderPath) 
    {
        ...
        const translations = this.#getTranslations('X:\\All\\7. Work\\Laungige Amir\\saves\\translations.json');
    ...
    async #pasteLanguagesMaterials_exAsync(audioFiles,translations,audioFolder,picturesFolder,mainAudioFile)
    {
        ...
        var translationHitLef = await ae.findCompositionByName('arrow');
        
        if (!translateComp) {
            throw new Error('I donna want to swear on you:( But you forgot add ',this.compositions.translationHitLeft,' ;( im sorry, my friend');
        }
        ...
        //for (let i = 0; i < 1; i++) {
            ...
            var endPositionAnimation = startPositionAnimation+0.8;
            currentTime += englishDuration + 0.8;
    ...
    #getMusic(audioFileName)
    {
        try {
            ...
            console.log("try get "+audioFileName);
            ...
            console.log("result selectedFile: "+selectedFile);
            console.log("result pathToFile: "+pathToFile);
    
            console.log("the end "+audioFileName);
    ...
    fonts = Object.freeze({
        ...
        Sinhala: "NotoSerifSinhala-Regular",
        Chinese: "NotoSansSC-Regular"
    });
